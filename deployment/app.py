...
# SendGrid integration (optional - falls back to SMTP if not available)
try:
...
SENDGRID_API_KEY = os.getenv('SENDGRID_API_KEY', None)  # Prefer SendGrid if API key is set
...
# Determine which email method to use (SendGrid preferred if API key is set)
USE_SENDGRID = SENDGRID_AVAILABLE and SENDGRID_API_KEY is not None
...
def send_email_via_sendgrid(to_email, subject, html_content, text_content):
    ...
    try:
        ...
        # Check if successful (status code 202 is success for SendGrid)
...
def send_admin_notification(name, phone, business_email, company, message):
    ...
    try:
        ...
        text_content = f"""🔥 NEW LEAD - ACTION REQUIRED
...
Please respond promptly to this lead.
        ...
        if USE_SENDGRID:
            ...
            return send_email_via_smtp(TO_EMAIL, subject, html_content, text_content, priority='high')
...
This is an automated confirmation email. Please do not reply to this message.
        ...
        # Use SendGrid if available, otherwise use SMTP
        if USE_SENDGRID:
            return send_email_via_sendgrid(business_email, subject, html_content, text_content)
        else:
            return send_email_via_smtp(business_email, subject, html_content, text_content)
...
if USE_SENDGRID:
    print("✓ Using SendGrid for email delivery", file=sys.stderr)
elif not SMTP_USERNAME or not SMTP_PASSWORD:
    print("WARNING: SMTP_USERNAME and SMTP_PASSWORD are not set in environment variables.", file=sys.stderr)
    print("WARNING: SENDGRID_API_KEY is also not set. Email sending may fail.", file=sys.stderr)
    print("Please set either SENDGRID_API_KEY or SMTP credentials in .env file or as environment variables.", file=sys.stderr)
else:
    print("✓ Using SMTP for email delivery", file=sys.stderr)

if not TO_EMAIL:
    print("WARNING: TO_EMAIL is not set. Contact form emails will not be sent anywhere.", file=sys.stderr)
